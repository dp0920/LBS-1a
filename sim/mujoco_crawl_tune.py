...
def rollout(model, params, n_strides=5, viewer=None):
    ...
    reward = dx - 2.0*mean_tilt - 5.0*max(0, -dz) - (50.0 if terminated else 0.0)
    return reward, dict(dx=dx, dz=dz, mean_tilt=mean_tilt, terminated=terminated)
...
def tune(out="best_params.json", generations=30, popsize=12, n_strides=5):
    ...
    while not es.stop():
        ...
        es.tell(xs, fs)
        gen += 1
        print(f"gen {gen:3d}  best_reward={best_r:+.3f}  pop_mean={-np.mean(fs):+.3f}")
