...
\vspace{0.3em}
\centering\includegraphics[width=0.78\textwidth]{ppo_learning_curves.png}
\end{frame}
...
\vspace{0.2em}
\centering\includegraphics[width=0.85\textwidth]{convergence_xconfig.png}
...
\textbf{Step-response on each LX-16A; fit MuJoCo PD to match.}

\vspace{0.3em}
\textbf{Pi:} \texttt{motor\_step\_response.py} steps each servo $\pm 10^\circ$, samples \texttt{get\_physical\_angle()} at 50\,Hz, saves JSON.\\
\textbf{Mac:} \texttt{fit\_pd\_from\_step\_response.py} sweeps $(k_p, k_v)$ for the best 2nd-order fit per servo.
...
\begin{enumerate}\setlength{\itemsep}{0pt}
    \item Original $k_p{=}2.5$ was \emph{too stiff} --- sim is a faster actuator than reality.
    \item RMSE of $5$--$7^\circ$ on a $10^\circ$ step won't go away --- \textbf{the LX-16A is a trajectory generator, not a PD}. No $(k_p, k_v)$ matches.
\end{enumerate}
\end{frame}
...
\vspace{0.5em}
\textbf{Tested (v35, 3M steps $\times$ 3 seeds, X-config + low-friction DR):}
\begin{itemize}
    \item Training looked great: \texttt{ep\_rew\_mean $\approx +10{,}500$}, \texttt{ep\_len\_mean $= 2000$} across all three seeds.
    \item Deterministic eval at $\mu{=}0.4$: \textbf{0/10 episodes survived; mean distance $-0.36$\,m (\emph{backward})}. Saturation rate stayed at 79\%.
    \item \textbf{The policy learned to stand, not walk.} The v20 reward gives $\sim 5$/step from alive + posture --- standing 2000 steps yields $\sim$+10\,k without moving. The slew limit removed the bang-bang shortcut to forward motion, leaving the standing-still attractor as the easier optimum.
\end{itemize}

\vspace{0.3em}
\textbf{What's needed to land deployable PPO:} a stronger forward-velocity bonus (or curriculum that learns walking first, then adds the slew limit), so the optimizer doesn't fall into the standing-still local minimum. \emph{The slew actuator is the right structural fix; reward shaping is the next dial.}
...
\vspace{0.5em}
\textbf{What's next, with v35's lesson in hand:}
\begin{itemize}
    \item Stronger forward-velocity bonus or curriculum (walk-first, then add slew limit) so PPO doesn't park in the standing-still attractor
    \item Higher slew rate ($\sim 250\,$dps, closer to LX-16A no-load) --- gives the optimizer more head-room while still preventing bang-bang
    \item Bench \emph{friction} calibration (same step-response idea applied to $\mu$)
\end{itemize}
